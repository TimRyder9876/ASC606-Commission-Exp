...
INSERT INTO SCHEMA.FINCOMM_PAY_PERIOD_13MTH
...
SELECT 
 ...
 GTM_MONTH_ID   
FROM
   SCHEMA.FINREPO_PAY_PERIOD_13MTH_VW@DATABASE.WORLD;
   
COMMIT; 
   

/***** LOAD ANY NEW PAY PERIODS INTO TMP_HR_JEID_PP *****/ 
INSERT INTO SCHEMA.COM_HR_JEID_PP
...
FROM 
 SCHEMA.FINCOMM_PAY_PERIOD_13MTH P
 LEFT JOIN SCHEMA.COM_HR_JEID_PP H
  ON
...
MERGE INTO SCHEMA.COM_HR_JEID_PP PP
USING
 (
  SELECT 
    P.PAY_PERIOD_ID AS PP_ID
  FROM 
   SCHEMA.FINCOMM_PAY_PERIOD_13MTH P
   INNER JOIN SCHEMA.COM_HR_JEID_PP H
    ON
     P.PAY_PERIOD_ID = H.PP_ID
  WHERE
   H.PROCESSED = 'N' AND
...
TRUNCATE TABLE SCHEMA.FINCOMM_SF_REP_TRANS;

INSERT INTO SCHEMA.FINCOMM_SF_REP_TRANS
...
FROM
  SCHEMA.FINREPO_SF_REP_TRANS_VW@DATABASE.WORLD T;
...
INSERT INTO SCHEMA.FINCOMM_NCCP_CUSTDET_RPT N
...
SELECT /*+ DRIVING_SITE(C) */
 ...
 DAR_NI_AMT, TYPE_NUM, PRODUCT_TYPE_TYPE, SORT_NUM, MONTH_ID
FROM 
 SCHEMA.FINREPO_NCCP_CUSTDET_RPT_VW@DATABASE.WORLD C;
COMMIT;                       

/***** LOAD FINREPO_PH_NCCP_PP_DETAIL_VW BASED ON SELECTED PAY PERIODS *****/
TRUNCATE TABLE SCHEMA.FINCOMM_PH_NCCP_PP_DETAIL;

INSERT INTO SCHEMA.FINCOMM_PH_NCCP_PP_DETAIL 
...
FROM
 SCHEMA.FINREPO_PH_NCCP_PP_DETAIL_VW@DATABASE.WORLD N;
...
TRUNCATE TABLE SCHEMA.FINCOMM_SCP_REP_LIST;

INSERT INTO SCHEMA.FINCOMM_SCP_REP_LIST
...
FROM 
 SCHEMA.FINREPO_SCP_REP_LIST_VW@DATABASE.WORLD R;
 
COMMIT;
                      
                                            
/***** RELOAD ALL FINREPO_PP_INCENTIVE_DATA_VW *****/                      
TRUNCATE TABLE SCHEMA.FINCOMM_PP_INCENTIVE_DATA;

INSERT INTO SCHEMA.FINCOMM_PP_INCENTIVE_DATA
 (EMPLOYEE_ID, ERN_PAY_PERIOD_DTE, ERN_CDE, ERN_DESC, ERN_AMT, REP_NUM)

SELECT
 EMPLOYEE_ID, ERN_PAY_PERIOD_DTE, ERN_CDE, ERN_DESC, ERN_AMT, REP_NUM
FROM 
 SCHEMA.FINREPO_PP_INCENTIVE_DATA_VW@DATABASE.WORLD;
COMMIT;
